        ...
        while True:
            ...
            if applying_force:
                if current_time - force_start_time >= force_duration:  # 力回饋時間到
                    ...
                    # 隨機更新直行時間範圍
                    random_duration = random.uniform(5.0, 10.0)
                    straight_duration_min = random_duration
                    straight_duration_max = random_duration + 1.0  # 確保最大值比最小值大
                    print(f"更新直行時間範圍: {straight_duration_min:.2f} - {straight_duration_max:.2f} 秒")
